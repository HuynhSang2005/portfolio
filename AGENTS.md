...
**CI vs deploy:** GitHub Actions owns the `quality` job (CI only, runs on every
PR and push, split steps for fail-fast). **Cloudflare Workers Builds** owns
deployment (CD): connected to this repository, it builds and deploys the Worker
on every push to `main`. There is no `deploy` job in GitHub Actions. See
`docs/deploy.md`.
...
- **Workers-true verification:** **Cloudflare Workers Builds** runs the OpenNext
  build (`npx opennextjs-cloudflare build`) and deploy (`npx
opennextjs-cloudflare deploy`) on every push to `main`, publishing to the
  canonical custom domain **`portfolio.huynhsang.id.vn`**. Local `bun run
deploy` is a break-glass fallback only.
