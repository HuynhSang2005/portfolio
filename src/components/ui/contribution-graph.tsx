...
/** Một ngày hoạt động trong heatmap — `level` từ 0 đến `maxLevel`. */
...
/** Nhãn tùy chỉnh cho tháng, ngày trong tuần, tổng đóng góp và legend. */
export type Labels = {
  months?: string[];
  weekdays?: string[];
  totalCount?: string;
...
/**
 * Root heatmap đóng góp GitHub-style — cung cấp context cho calendar, block, footer.
 * Trả về `null` khi `data` rỗng.
 */
...
/** Ô SVG một ngày — `data-level` quyết định màu fill. */
...
/** Lưới SVG tuần × ngày — render prop cho từng block. */
...
/** Hàng footer — chứa total count và legend. */
...
/** Hiển thị tổng đóng góp — interpolate `{{count}}` và `{{year}}` từ labels. */
...
/** Legend Less → More với các ô mức độ. */
      ...
      {Array.from({ length: maxLevel + 1 }, (_, level) =>
        children ? (
          ...
          <svg height={blockSize} key={level} width={blockSize}>
            ...
            <rect
              className={LEVEL_FILL_CLASSES[level] ?? LEVEL_FILL_CLASSES[0]}
              height={blockSize}
              rx={blockRadius}
              ry={blockRadius}
              width={blockSize}
