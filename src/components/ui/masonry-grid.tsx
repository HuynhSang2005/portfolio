...
function createHashFromInput(input: string): string {
...
`.trim();
  ...
  return (
    <div className={`relative mx-0 my-auto mb-20 flex flex-col pt-2 pl-1 ${className}`}>
      <style dangerouslySetInnerHTML={{ __html: styleContent }} suppressHydrationWarning />
      <div className={`${masonryClassId} w-auto pr-2 pl-1`}>{wrappedChildren}</div>
    </div>
  );
};
