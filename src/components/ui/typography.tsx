...
/** Wrapper prose Tailwind Typography — lớp modifier template cho nội dung MDX/blog. */
...
/** Biến thể {@link Prose} nhỏ hơn, font mono — dùng cho dossier/readout. */
...
/** Code inline (pill) hoặc block (`data-language`) — block không ghi đè style pill. */
...
type HeadingProps<T extends HeadingTypes> = React.ComponentProps<T> & {
  /** Thẻ heading HTML (mặc định `h1`). */
  as?: T;
};

/**
 * Heading với anchor permalink khi có `id` — icon link hiện khi hover tiêu đề.
 */
