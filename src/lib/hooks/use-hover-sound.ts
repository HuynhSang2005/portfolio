...
/**
 * Tiếng "whoosh" nhẹ khi hover — DSP template verbatim (bandpass 1600Hz, throttle 50ms).
 *
 * @returns Hàm `play()` — gọi khi hover; no-op khi âm thanh tắt.
 */
export function useHoverSound() {
  ...
  const play = useCallback(() => {
    ...
    // Throttle: không phát nếu chưa đủ 50ms kể từ lần trước
    const now = Date.now();
    ...
    try {
      ...
      if (!noiseBufferRef.current) {
        ...
        // Pink-ish noise cho âm thanh mềm hơn
        let b0 = 0,
          b1 = 0,
          b2 = 0,
          b3 = 0,
