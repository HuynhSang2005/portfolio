...
4. **Contact**: form + Resend email + Turnstile + Workers rate limiter (new surface, no template counterpart).
   Workers-true smoke via **Cloudflare deploy** (`portfolio.huynhsang.id.vn`), not mandatory local `bun run preview`.
   — [spec](superpowers/specs/2026-08-03-p4-contact-design.md) · [plan](superpowers/plans/2026-08-03-p4-contact.md)
5. **Extras** (each re-confirmed): craft experiments (`cal-grid` / `vercel-grid`), craft feed/llms, clock, buddy, cal, analytics, and the deferred canonical-subdomain sitemap submission/monitoring runbook. **Note (2026-08-03):** craft masonry index + `[slug]` pulled into P4; remaining craft extras stay here. **Note (2026-08-04):** canonical origin is `portfolio.huynhsang.id.vn`; the apex is a path/query-preserving 308 redirect only.
   — [spec](superpowers/specs/2026-08-04-p5-extras-design.md) · [plan](superpowers/plans/2026-08-04-p5-extras.md)
